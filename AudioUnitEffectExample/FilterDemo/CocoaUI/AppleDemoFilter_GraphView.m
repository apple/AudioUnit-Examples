/*
<samplecode>
<import>AppleDemoFilter_GraphView.h</import>
</samplecode>
*/
...
- (id)initWithFrame:(NSRect)frameRect
{
	if ((self = [super initWithFrame:frameRect]) != nil) {
		...
		mGraphFrame =
			NSMakeRect(kDBAxisGap, // Initialize frame that is used for drawing the graph content
				kFreqAxisGap, frameRect.size.width - kDBAxisGap - kRightMargin,
		...
		[dbLabelStyle setAlignment:NSTextAlignmentRight];

		mDBAxisStringAttributes = [NSDictionary
			dictionaryWithObjectsAndKeys:[NSFont systemFontOfSize:9], NSFontAttributeName,
			dbLabelStyle, NSParagraphStyleAttributeName,
			[NSColor colorWithDeviceWhite:.1 alpha:1], NSForegroundColorAttributeName, nil];
		...
		[freqLabelStyle setAlignment:NSTextAlignmentCenter];

		mFreqAxisStringAttributes = [NSDictionary
			dictionaryWithObjectsAndKeys:[NSFont systemFontOfSize:9], NSFontAttributeName,
			freqLabelStyle, NSParagraphStyleAttributeName,
			[NSColor colorWithDeviceWhite:.1 alpha:1], NSForegroundColorAttributeName, nil];

		mEditPoint = NSZeroPoint;
		mActiveWidth =
			[self locationForFrequencyValue:kDefaultMaxHertz] - mGraphFrame.origin.x - .5;
...
- (void)drawDBScale
{
	...
	for (index = -kNumDBLines; index <= kNumDBLines; index++) {
		...
		startPoint = NSMakePoint(mGraphFrame.origin.x,
			floor([self locationForDBValue:index * (kDefaultGain / kNumDBLines)]) + .5);
		...
		[[NSString localizedStringWithFormat:@"%d db", value]
				drawInRect:NSMakeRect(0, startPoint.y - 4, mGraphFrame.origin.x - 4, 11)
...
- (void)drawMajorGridLines
{
	...
	for (index = 0; index <= kNumGridLines; index++) {
		...
		if (index > 0 && index < kNumGridLines) {
			[gridColor set];
			[NSBezierPath
				strokeLineFromPoint:NSMakePoint(location, mGraphFrame.origin.y)
							toPoint:NSMakePoint(location,
										floor(mGraphFrame.origin.y + mGraphFrame.size.height - 2) +
											.5)];
			...
			[s drawInRect:NSMakeRect(location - 3 - labelWidth / 2, 0, labelWidth, 12)
			...
			[[[self stringForValue:value] stringByAppendingString:@"Hz"]
					drawInRect:NSMakeRect(location - labelWidth / 2, 0, labelWidth, 12)
				withAttributes:mFreqAxisStringAttributes];
			...
			[[[self stringForValue:kDefaultMaxHertz] stringByAppendingString:@"K"]
					drawInRect:NSMakeRect(
								   location - labelWidth / 2 - 12, 0, labelWidth + kRightMargin, 12)
				withAttributes:mFreqAxisStringAttributes];
		}
	}
}
...
- (void)drawControlPoint
{
	...
	NSRect hLine = NSIntegralRect(NSMakeRect(
		mGraphFrame.origin.x, mEditPoint.y, (mEditPoint.x - 3) - mGraphFrame.origin.x, 1));
	NSFrameRect(hLine);
	...
	NSRect vLine = NSIntegralRect(NSMakeRect(
		mEditPoint.x, mGraphFrame.origin.y, 1, (mEditPoint.y - 3) - mGraphFrame.origin.y));
	NSFrameRect(vLine);
...
/* ------- NOTES ON DRAWING -------
	For the purposes of this sample, we do only the most basic performance optimizations in the
   interest of keeping this sample reasonably simple such as caching the background graph and labels
   in an image.

	There are several additional optimizations that could be performed in order to enhance graphic
   speed

	1) This is a non-opaque view. Every time it draws, it is necessary to redraw the window
   background before drawing the view contents. Drawing a solid fill color in the background of the
   view and overriding -(void) isOpaque to return YES would result in some speedup 2) The view is
   drawn anti-aliased. It is probably unneccesary to draw anti-aliased when the mouse is being
   dragged. Calling setShouldAntialias: NO on the NSGraphicsContext when the mouse is dragging
   begins, and setting it back to YES when dragging ends could result in a substatial speed
   increase. Likewise, the control point could always be drawn with anti-aliasing off because it is
   drawn aligned to pixel boundaries 3) Drawing the curve could be done with Quartz to avoid the
   overhead of NSBezierPath 4) The curve should be drawn with increased resolution around the
   control point and less resolution farther away. The fewer points in the curve plot, the faster it
   will draw 5) Drawing the curve without transparency may increase the render time

	Remember that before doing any optimization, make sure that your code is working correctly
   first, and then profile with Shark to determine which areas could benefit from the most
   optimization. Premature optimization can actually result in slower code.
*/
- (void)drawRect:(NSRect)rect
{
	if (!mBackgroundCache) {
		...
		NSRectFill(NSIntersectionRect(rect, NSMakeRect(mGraphFrame.origin.x, mGraphFrame.origin.y,
												mActiveWidth, mGraphFrame.size.height)));

		// draw the graph border
		[[NSColor whiteColor] set];
		NSRect lineRect =
			NSMakeRect(mGraphFrame.origin.x, mGraphFrame.origin.y - 1, mActiveWidth, 1);
	...
	[mBackgroundCache drawInRect:rect fromRect:rect operation:NSCompositingOperationSourceOver fraction:1.0];
...
- (void)handleMouseEventAtLocation:(NSPoint)location
{
	...
	if (isInside) {
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:kGraphViewDataChangedNotification
															object:self];
	}
}
...
- (void)mouseDown:(NSEvent*)theEvent
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:kGraphViewBeginGestureNotification
														object:self];
...
{
	mMouseDown = NO;
	[[NSNotificationCenter defaultCenter] postNotificationName:kGraphViewEndGestureNotification
														object:self];
...
/*	This method is called to set the frequency response values in the data that correspond to the
   points that we will will later be drawing The data argument is an array of FrequencyResponse
   structures. The number of items in this array is a fixed size, so we have a finite amount of
   resolution. We compute a pixelRatio which specifies how many pixels separate each frequency value
*/
- (AudioUnitFrequencyResponseBin*)prepareDataForDrawing:(AudioUnitFrequencyResponseBin*)data
{
	...
	for (i = 0; i < kNumberOfResponseFrequencies; i++) {
		if (location > rightEdge) // if we have exceeded the right edge of our graph, just store the
								  // max hertz value
			data[i].mFrequency = kDefaultMaxHertz;
		else {
			float freq = [self
				freqValueForLocation:location]; // compute the frequency value for our location
			if (freq > kDefaultMaxHertz) // check to make sure our computed value does not exceed
										 // our maximum hertz value
...
- (void)plotData:(AudioUnitFrequencyResponseBin*)data
{
	// NOTE that much of this data could be cached since it will be the same every time we draw as
	// long as our frame size has not changed We do not do this optimization in the interest of
	// simplicity.
	float width = mActiveWidth;
	float rightEdge = width + mGraphFrame.origin.x;
	int i,
		pixelRatio = (int)ceil(
			width / kNumberOfResponseFrequencies); // compute how many pixels separate each db value
	float location = mGraphFrame.origin.x;

	if (!curveColor)
		curveColor = [NSColor colorWithDeviceRed:.31 green:.37 blue:.73 alpha:.8];

	mCurvePath = [NSBezierPath bezierPath]; // create a new default empty path
	[mCurvePath
		moveToPoint:mGraphFrame
						.origin]; // start the bezier path at the bottom left corner of the graph
	...
	for (i = 0; i < kNumberOfResponseFrequencies; i++) {
		...
		float dbPos = 0;
		if (dbValue <
			-kDefaultGain) // constrain the current db value to our min and max gain interval
			dbPos = mGraphFrame.origin.y;
		else if (dbValue > kDefaultGain)
			dbPos = mGraphFrame.origin.y + mGraphFrame.size.height;
		else
			dbPos = [self locationForDBValue:dbValue]; // if the current db value is within our
													   // range, compute the location

		if (fabsf(lastDBPos - dbPos) >=
			.1) // only create a new point in our bezier path if the current db pixel value
			[mCurvePath
				lineToPoint:NSMakePoint(location,
								dbPos)]; // differs from our previous value by .1 pixels or more
	...
	[mCurvePath
		lineToPoint:NSMakePoint(location,
						mGraphFrame.origin
							.y)]; // set the final point to the lower right hand corner of the graph
...
{                     // called when parameter automation started
	mMouseDown = YES; // simulate physical mouse press in the view
	[self setNeedsDisplay:YES];
}

- (void)handleEndGesture
{                    // called when parameter automation finished
	mMouseDown = NO; // simulate mouse up in the view
	[self setNeedsDisplay:YES];
}

- (void)disableGraphCurve
{ // update the view, but don't draw the curve (used when the AU is not initialized and the curve
