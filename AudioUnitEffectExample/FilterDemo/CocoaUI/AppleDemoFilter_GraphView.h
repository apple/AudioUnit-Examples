/*
<samplecode>
<abstract>AU Filter Graph View</abstract>
</samplecode>
*/
...
@interface AppleDemoFilter_GraphView : NSView {
	...
	NSImage* mBackgroundCache; // An image cache of the background so that we don't have to re-draw
							   // the grid lines and labels all the time
	...
	NSDictionary*
		mDBAxisStringAttributes; // Text attributes used to draw the strings on the db axis
	NSDictionary*
		mFreqAxisStringAttributes; // Text attributes used to draw the strings on the frequency axis
}
...
- (double)locationForFrequencyValue:
	(double)value; // converts a frequency value to the pixel coordinate in the graph
- (double)locationForDBValue:
	(double)value; // converts a db value to the pixel coordinate in the graph

- (AudioUnitFrequencyResponseBin*)prepareDataForDrawing:
	(AudioUnitFrequencyResponseBin*)data; // prepares the data for drawing by initializing frequency
										  // fields based on values on pixel boundaries
- (void)plotData:(AudioUnitFrequencyResponseBin*)data; // draws the curve data
- (void)disableGraphCurve; // update the view, but don't draw the curve (used when the AU is not
						   // initialized and the curve can not be retrieved)
